...
import com.etlegacy.app.q3e.Q3EKeyCodes;
import com.etlegacy.app.q3e.Q3EUtils;
import com.etlegacy.app.q3e.gl.KGLBitmapTexture;
import com.etlegacy.app.q3e.gl.Q3EGL;
...
public class Joystick extends Paintable implements TouchListener
{
    ...
    public void setenabled(int ind, boolean b)
    {
        if ((keys[ind] != b))
        {
            ...
            if (b)
            {
                //SDLControllerManager.onNativePadDown(0, (Q3EUtils.q3ei.callbackObj.notinmenu ? codes : Menu_Codes)[ind]);
                SDLActivity.onNativeKeyDown((Q3EUtils.q3ei.callbackObj.notinmenu ? codes : Menu_Codes)[ind]);
            }
            else
            {
                //SDLControllerManager.onNativePadUp(0, (Q3EUtils.q3ei.callbackObj.notinmenu ? codes : Menu_Codes)[ind]);
                SDLActivity.onNativeKeyUp((Q3EUtils.q3ei.callbackObj.notinmenu ? codes : Menu_Codes)[ind]);
            }
...
//                    if (Math.abs(analogy) > 1.0f) analogy = analogy / Math.abs(analogy);
                    ...
                    {
                        //m_pressed = true;
                        //SDLControllerManager.onNativeJoy(0, ,analogx, analogy);
                        SDLActivity.onNativeTouch(0, 0, MotionEvent.ACTION_MOVE, analogx, analogy, 1);
                        //Q3EUtils.q3ei.callbackObj.sendAnalog(true, analogx, analogy);
                    }
                    else
                    {
                        //m_pressed = false;
                        SDLActivity.onNativeTouch(0, 0, MotionEvent.ACTION_UP, 0, 0, 0);
                        //SDLControllerManager.onNativeJoy(0, analogx, analogy);
...
/*
                            ...
                            */
                        if(NotInDeadZone(deltax, deltay))
                            SDLActivity.onNativeTouch(0, 0, MotionEvent.ACTION_MOVE, deltax, deltay, 1);
                            //Q3EUtils.q3ei.callbackObj.sendAnalog(true, analogx, analogy);
                        else
                            SDLActivity.onNativeTouch(0, 0, MotionEvent.ACTION_UP, 0, 0, 0);
                            //Q3EUtils.q3ei.callbackObj.sendAnalog(false, 0, 0);
